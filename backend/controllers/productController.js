...
const updateStockAfterOrder = async (orderItems) => {
  ...
  for (const item of orderItems) {
    stockDecrements[item._id] =
      (stockDecrements[item._id] || 0) + item.quantity;
  }
  ...
  if (updates.length > 0) {
    ...
    if (result.modifiedCount !== updates.length) {
      ...
      throw new Error(
        `CRITICAL STOCK ERROR: ${failedUpdates} item(s) failed the stock check. Cannot complete order.`
      );
    }
  }
...
const restoreStockAfterCancellation = async (orderItems) => {
  ...
  for (const item of orderItems) {
    ...
    stockIncrements[item._id] =
      (stockIncrements[item._id] || 0) + item.quantity;
  }
